...
document.addEventListener('DOMContentLoaded', function(){
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeM();closeConf();closeAulasModal();closeUbicacionesModal();closePrestar();closeDevolver();closeProfModal();closeImport();closeExportModal();closeDocsModal();closeDelModal();closeHistorial();closeQrScanner();closeUsuariosModal();closeModulosUsuario();closePrintModal();closeTourCamara();closeAyudaCamara();closeCamaraSerie();closeCamaraUnificada();closeReservaPractica();closeCatsModal();closeAtencionHoyModal();closeAuditoriaModal();closePrintLabelsModal();closePrintChoiceModal();closeStockChoiceModal();closeMisAulasModal();closeAccesosModal();closeHistorialModal();closeConfigBackups();closePrestarPicker();closePrestarCaja();closePresDevModal();closeAulasUsuario();closeMantenimientoUsuario();closeMultiEquipo();closePhotoModal();closePrintQrModal();closeQrSimpleModal();closeItemQr();closeBaja();closePedidos();closeCiclosModal();closeRevisionAula();closeMisModulosModal();closeSolicitudesModal();closeMisMantModal();closeMobMenu();closeDeptMenu();closeMisCursosMenu()}});
